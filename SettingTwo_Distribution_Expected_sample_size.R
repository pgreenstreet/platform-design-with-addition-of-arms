...
PESSD=function(L,U,sh,S,r,r0,N,delta,n,n0,acm,lsh,sd,k,q)
{ 
  shstar=max(sh)
  ...
  for(i in 1:(N^S))
  {
      ...
      probbit[i]=normpart*wei*in_int(L=L,U=U,sh=sh,S=S,r=r,r0=r0,delta=delta,n=n,n0=n0,t=tt[(S*(i-1)+1):(S*i)],acm=acm,lsh=lsh,sd=sd,k = k,q=q)#function given value # in this we have egnored the dphi(t) as this cancels with the normal distribution
  }
...
Expectedsamplesize=function(sh,S,r,r0,N,gn,U,L,delta,sd)
{
  ...
  for(f in 1:lsh)
  {
    ptr=which(abs(perplist[,f]) > (S-sh[f]) ) #lines to remove
    #print("ptr")
    #print(ptr)
    if(length(ptr)>0)
    {  
      perplist=perplist[-ptr,]
    }
    #print(perplist)
  }
 
  d1pl=dim(perplist)[1] #dim 1 of perplist
  ...
  for(i in 1:d1pl)
  {
    ...
    k=abs(cpl)
    qm11= sign(cpl) #q with 1 and -1 in it
    q=replace(qm11,qm11==-1,Inf)
    cntpart=ntpart(n=n,n0=n0,sh=sh,lsh=lsh,k=k,q=q)
    cPESSD=PESSD(L=L,U=U,sh=sh,S=S,r=r,r0=r0,N=N,delta=delta,n=n,n0=n0,acm=acm,lsh=lsh,sd=sd,k=k,q=q)
    resultmatrix[i,]=c(cntpart,cPESSD)
  }
  return(resultmatrix)
}

load("threeStritri.RData")
boundsandn=threeStritri[[1]]
n1=boundsandn[[1]]
n2=boundsandn[[2]]
nt=matrix(c(n1,2*n1,3*n1,n2,2*n2,NA),nrow = 2,ncol = 3, byrow = T)
n0t=c(n1,n1+max(n1,n2),n1+max(n1,n2)+max(n1,n2))
Ut=boundsandn[[3]]
Lt=boundsandn[[4]]
gnt=1

#gn is given n per 1 ratio 
ptm <- proc.time()
set.seed(1)
ansNull=Expectedsamplesize(gn=gnt,sh=c(0,1),S=3,r=nt,r0=n0t,N=20,U=Ut,L=Lt,delta=c(0,0),sd=1)
sum(ansNull[,4]*ansNull[,5]) #expected sample size
set.seed(1)
ansT1=Expectedsamplesize(gn=gnt,sh=c(0,1),S=3,r=nt,r0=n0t,N=20,U=Ut,L=Lt,delta=c(-log(0.69),-log(0.99)),sd=1)
sum(ansT1[,4]*ansT1[,5]) #expected sample size
set.seed(1)
ansT2=Expectedsamplesize(gn=gnt,sh=c(0,1),S=3,r=nt,r0=n0t,N=20,U=Ut,L=Lt,delta=c(-log(0.99),-log(0.69)),sd=1)
sum(ansT2[,4]*ansT2[,5]) #expected sample size
proc.time() - ptm

sampledis3tritri=list(ansNull,ansT1,ansT2)


#the null sample size distribution
ans=sampledis3tritri[[1]]
...
plot(TotalN_dataTN$TN,cumsum(TotalN_dataTN$PN),xlim=c(90,500),xlab="",ylab="",ylim=c(0,1),type = "s",lwd = 2)
...
plot(TotalN_dataCT$CT,cumsum(TotalN_dataCT$PN),xlim=c(0,200),ylim=c(0,1),xlab="",ylab="",type = "s",col="Blue",lwd = 2)
