...
editedboundpower=function(L,U,sh,S,r,r0,n,n0,delta0,f,delta1,sd,t,lsh)
{
  ...
  for(h in 1:lsh)
  {
    ...
    for(j in 1:S) 
    {
      partl=L[h,j]*sqrt(1+(r[h,j]/(r0[(sh[h]+j)]-r0sh))) 
      partu=U[h,j]*sqrt(1+(r[h,j]/(r0[(sh[h]+j)]-r0sh))) 
...
corralationmatixf=function(r,f,J) 
{
  ...
  for(j1 in 1:(J-1)) #j
  {
    for(j2 in 1:(J-1)) #j*
    {
      inside1=(r[f,j1]*(r[f,J]-r[f,j2]))/(r[f,j2]*(r[f,J]-r[f,j1])) 
      inside2=(r[f,j2]*(r[f,J]-r[f,j1]))/(r[f,j1]*(r[f,J]-r[f,j2]))  
...
treatmentfatJ=function(r,f,J,eb,v,fcm,lsh) 
{
  startpart=1
  if(J>1)
  {
    cm=fcm[[((J-2)*lsh+f)]]
    ebf=editedboundf(f=f,eb=eb,r=r,v=v,J=J) 
    lebf= ebf[[1]] 
    uebf= ebf[[2]] 
    if(J == 2)
    {
...
{ 
  ...
  BJ=treatmentfatJ(r=r,f=f,J=J,eb = eb,v = v,fcm=fcm,lsh=lsh) 
