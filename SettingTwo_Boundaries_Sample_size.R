...
editedbound=function(L,U,sh,S,r,r0,t,lsh)
{
  ...
  for(h in 1:lsh)
  {
    ...
    for(j in 1:(S-sh[h])) 
    {
      ...
      partt1=sqrt(r[h,j])/(r0[(sh[h]+j)]-r0sh) # the t part at the beginning
...
ALPhaforgivent=function(L,U,sh,S,r,r0,t,acm,lsh)
{
  ...
  for(h in 1:A)
  {
    ...
    sumpart=rep(NA,S-sh[h]) #sum part
...
FWER=function(L,U,sh,S,r,r0,N,acm,lsh)
{ 

  quad=gausspoints(N=N, -6, 6)
  perm=permutations(n=N, r=S,repeats.allowed = TRUE)
  ...
  for(i in 1:(N^S))
  {
    fv=ALPhaforgivent(L,U,sh,S,r,r0,t=tt[(S*(i-1)+1):(S*i)],acm,lsh)#function given value # in this we have egnored the dphi(t) as this cancels with the normal distribution
    wei=prod(tw[(S*(i-1)+1):(S*i)]) #weight part
    normpart=prod(dnorm(tt[(S*(i-1)+1):(S*i)]))
...
corralationmatixfet=function(r,r0,sh) #given h # correlation matrix for each treatment
{
  ...
  for(h in 1:A)
  {  
    Cormat=diag(1,nrow = (S-sh[h]))
    for(j1 in 1:(S-sh[h])) #j
    {
      for(j2 in 1:(S-sh[h])) #j*
      {
        if(sh[h]==0)
        {
          r0sh=0  
...
currentagivenhUL=function(acm,sh,L,U,f,S,seed.number)
{
  ...
  sumpart=rep(NA,S-sh[f]) #sum part
...
boundfinder=function(f,sh,S,r,r0,N,alpha,tolerance,power,sd,aratio,amax,BshL,BshU)
{
  ...
  halfpointvalue=FWER(L,U,sh,S,r,r0,N=N,acm=acm,lsh=lsh)
  counter=1
...
{
  leb= eb[[1]] #lower edited boundaries 
...
corralationmatixf=function(r,f,J) #correlation matrix for f given j
{
...
{ 
  ...
  lt=sh[f]+J #the number of t we will integrate over 
...
# find power under least variable configuration for every f
LFCPowerf=function(L,U,sh,S,r,r0,n,n0,delta0,delta1,sd,N,acm,lsh,fcm,f)
{
  Jpower=rep(NA,(S-sh[f]))
  for(j in 1:(S-sh[f]))
  {
...
findboundMamsandn=function(sh,S,N,alpha,tolerance,power,delta0,delta1,sd,nmax,amax)
{
  ...
  n1=1
  n2=1
  r=matrix(c(n1,2*n1,3*n1,n2,2*n2,NA),nrow = lsh,ncol = S, byrow = T) #need to change this code here for different number of stages or arms
  r0=c(n1,n1+max(n1,n2),n1+max(n1,n2)+max(n1,n2)) #need to change this code here for different number of stages or arms
  
  acm=corralationmatix(r) #all correlation matrices
  alist=c() #to stop it going into an invite loop if by changing the design it keeps jumping (it will stick with the worse U) 
  ...
  nl=0 #minimum value for n  #will tell you how much to times r by #put as 49 to speed up code
  nu=nmax
  i1=(nu+nl)/2
  n1=i1
  n2=i1
  n=matrix(c(n1,2*n1,3*n1,n2,2*n2,NA),nrow = lsh,ncol = S, byrow = T) #need to change this code here for different number of stages or arms
  n0=c(n1,n1+max(n1,n2),n1+max(n1,n2)+max(n1,n2))#need to change this code here for different number of stages or arms
  ...
  while(round(Oi1) != round(Ci1) | round(Oi2) != round(Ci2))
  {
    ...
    {  
      if(cp<power)
      {
        nl=i1 #minimum value for n  #will tell you how much to times r by #put as 49 to speed up code
        nu=nu
        i1=(nu+nl)/2
        n1=i1
        n2=n2
      ...
      if(cp>power)
      {
        nl=nl #minimum value for n  #will tell you how much to times r by #put as 49 to speed up code
        nu=i1
        i1=(nu+nl)/2
        n1=i1
        n2=n2
...
ptm=proc.time()
set.seed(1)
threeStritri=gcEBS(sh=c(0,1),S=3,N=20,alpha=0.025,tolerance=0.0001,power=0.8,delta0=-log(0.99),delta1=-log(0.69),sd=1,nmax=400,amax=40)
threeStritri
proc.time() - ptm

save(threeStritri,file = "threeStritri.RData")
